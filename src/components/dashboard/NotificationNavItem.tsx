...
export default function NotificationNavItem({ userId, pathname }: NotificationNavItemProps) {
    ...
    return (
        <div className="relative flex flex-col items-center justify-center min-w-[44px]">
            ...
            >
                {/* Bell SVG */}
                <div className={`flex items-center justify-center transition-colors ${activeCount > 0 ? 'text-orbit-text' : 'text-orbit-muted hover:text-orbit-text'}`} style={{ width: '22px', height: '22px' }}>
                    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                        <path d="M18 8a6 6 0 0 0-12 0c0 7-3 9-3 9h18s-3-2-3-9" />
                        <path d="M13.73 21a2 2 0 0 1-3.46 0" />
                    </svg>
                </div>
                <span className={`type-meta mt-1 truncate max-w-full ${activeCount > 0 ? 'text-orbit-text' : 'text-orbit-muted'}`}>Notifications</span>
